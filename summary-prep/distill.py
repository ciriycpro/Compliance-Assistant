...
Задачи:
1. Определи тип документа: bank_response, tax_declaration, contract, act,
   statement, letter, notice, report, other.
2. Извлеки 3-5 ключевых фактов (key_facts) — это что важно знать получателю.
3. Если есть конкретные суммы (с понятной ролью: tax/fine/amount_total/balance/other)
   и даты (document_date/due_date/period_start/period_end/other) — укажи.
4. Назови основные стороны (parties): отправитель, получатель, третьи лица.
5. Что требуется от получателя (requirements) — действия, сроки.
...
Задачи:
1. Итоговый document_type (большинство голосов).
2. summary_brief — 1-2 предложения для дайджеста: что это за документ, для кого
   и его суть. Чтобы получатель сразу понял что пришло.
3. Объедини key_facts из чанков, дедуплицируй. ≤ 5-7 итоговых фактов.
4. Объедини amounts/dates/parties/requirements, дедуплицируй.
5. confidence — средневзвешенный.
